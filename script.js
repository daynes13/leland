...
function showQuestion() {
    ...
    question.answers.forEach((answer, index) => {
        ...
        answerDiv.onclick = () => selectAnswer(answerDiv, answer, index);
        answerButtons.appendChild(answerDiv);
    });

    const nextButton = document.getElementById('next-button');
    nextButton.innerText = currentQuestionIndex === questions.length - 1 ? "Submit" : "Continue";
...
function nextQuestion() {
    ...
    if (currentQuestionIndex === questions.length - 1) {
        // Redirect to the appropriate results page based on scores
        if (greScore > gmatScore) {
            window.location.href = 'greresults.html';
        } else {
            window.location.href = 'gmatresults.html';
        }
    } else {
